...
# mutoh: don't use aliaswiki
       @aliaswiki = AliasWiki::new("")
...
#      @p = @aliaswiki.original_name(@p).to_euc if @p
  ...
  private
    def generate_page( data, status = nil )
      status ||= "OK"
    ...
    def cmd_view
      ...
      unless tokens
        @db.set_last_update( @p, File.mtime( @db.send( :textdir, @p )))
        text = @db.load( @p )
    ...
    def cmd_edit( page, text=nil, msg=nil, d_title=nil, status=nil )
    ...
    def cmd_save( page, text, md5hex, update_timestamp = true )
      ...
      if text.empty?
        ...
        if Filter.new_page_is_spam?(page, text, title)
          @cmd = 'is_spam'
          cmd_edit( page, text, @conf.msg_input_is_spam, nil, "403 Forbidden" )
          return
        end
